                    ...
                    if os.path.isfile(os.path.join(study_dir, file)):
                        ...
                        for subfile in os.listdir(subdir):
                            if os.path.isfile(os.path.join(subdir, subfile)):
                                ...
                                if subfile.lower().endswith(
                                        ('.spar', '.7', '.dat', '.ima', '.rda', '.nii', '.nii.gz', 'method')):
                                    tests[f'spec2nii_{vendor}_{study}_{file}_{subfile}'] = {
                                        'import_fpath': os.path.join(subdir, subfile)}
                                    # break
                                ...
                                for subsubfile in os.listdir(subsubdir):
                                    ...
                                    if subsubfile.lower().endswith(
                                            ('.spar', '.7', '.dat', '.ima', '.rda', '.nii', '.nii.gz', 'method')):
                                        tests[f'spec2nii_{vendor}_{study}_{file}_{subfile}_{subsubfile}'] = {
                                            'import_fpath': os.path.join(subsubdir, subsubfile)}
                                        # break
